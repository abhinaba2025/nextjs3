  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pt-20 pb-16">
      ...
      <div className="relative h-64 md:h-80 overflow-hidden">
        <img
          src="https://images.unsplash.com/photo-1550009158-9ebf69173e03?w=1920&h=600&fit=crop"
          alt="Electronics"
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          >
            ...
            <p className="text-lg md:text-xl text-white/80 mb-4">
              Discover cutting-edge technology and gadgets
            </p>
            <div className="flex items-center justify-center gap-2 text-sm">
              <Link to="/" className="hover:text-indigo-300 transition-colors">Home</Link>
              <span>/</span>
              <Link to="/shop" className="hover:text-indigo-300 transition-colors">Shop</Link>
              <span>/</span>
              <span className="text-indigo-300">Electronics</span>
            </div>
          </motion.div>
        </div>
      </div>
          ...
          >
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg sticky top-24">
              ...
              <div className="space-y-6">
                {/* Subcategories */}
                <div>
                  <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Subcategories</h4>
                  <div className="space-y-2">
                    {['All Electronics', 'Headphones', 'Smartwatches', 'Speakers', 'Laptops', 'Accessories'].map((sub) => (
                      <label key={sub} className="flex items-center gap-2 cursor-pointer group">
                        <input type="checkbox" defaultChecked={sub === 'All Electronics'} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                        <span className="text-sm text-slate-600 dark:text-slate-400 group-hover:text-indigo-600 transition-colors">{sub}</span>
                      </label>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {['All Brands', 'TechPro', 'SoundMax', 'SmartGear', 'ElectraX'].map((brand) => (
                      <label key={brand} className="flex items-center gap-2 cursor-pointer group">
                        <input type="checkbox" defaultChecked={brand === 'All Brands'} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                        <span className="text-sm text-slate-600 dark:text-slate-400 group-hover:text-indigo-600 transition-colors">{brand}</span>
                      </label>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    {[4, 3, 2, 1].map((rating) => (
                      <label key={rating} className="flex items-center gap-2 cursor-pointer group">
                        <input type="radio" name="rating" className="w-4 h-4 border-slate-300 text-indigo-600 focus:ring-indigo-500" />
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6 bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm">
              <p className="text-slate-600 dark:text-slate-400">
                Showing <span className="font-semibold text-indigo-600">{electronicsProducts.length}</span> products
              </p>
            ...
            >
              <Link
                to="/shop"
                className="inline-flex items-center gap-2 px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-full transition-colors"
              >
                View All Products
...
export default ShopElectronicsPage;
